 <div class="offcanvas offcanvas-start" tabindex="-1" id="offcanvasSidebar">
                 ...
                 <ul class="nav flex-column mb-auto">
                   ...
                   <li>
                        <a href="my_orders.php" class="nav-link d-flex justify-content-between align-items-center">
                            ...
                            <span id="offcanvas-order-incident-alert" class="badge bg-danger rounded-pill" style="display: none;">!</span>
                        </a>
                    </li>
      ...
      <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const checkIncidentsOffcanvas = async () => {
                try {
                    const response = await fetch('api/check_user_incidents.php');
                    const result = await response.json();
                    if (result.status === 'success' && result.has_incident) {
                        const alertIcon = document.getElementById('offcanvas-order-incident-alert');
