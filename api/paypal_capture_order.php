...
try {
    ...
    $stmt_payment->bind_param("ds", $validated_total, $capture_data['id']);
    $stmt_payment->execute();
    $payment_id = $stmt_payment->insert_id;

    // 2. Create the order in the product_orders table
    $stmt_order = $conn->prepare("INSERT INTO product_orders (user_id, total_amount, status, shipping_address_id, payment_id) VALUES (?, ?, 'paid', ?, ?)");
    $stmt_order->bind_param("idii", $user_id, $validated_total, $shipping_address_id, $payment_id);
    $stmt_order->execute();
